...
#include <iostream>
BYTE* HookEngine::getBufferWithOriginFun(LPVOID pOriginFun)
{
	LPVOID pNewFun = m_mapOriginFunToNewFun[pOriginFun];
	return m_mapNewFunToOriginFunPrologBuf[pNewFun].data();
}

bool HookEngine::restoreOriginFun(LPVOID pOriginFun, BYTE jmpBuffer[16])
{
...
bool HookEngine::restoreHook(LPVOID pOriginFun, BYTE jmpBuffer[16])
{
	//set read write permits 
	DWORD oldProtect = 0, dummy = 0;
	ASSERT_BOOL(::VirtualProtect(pOriginFun, JMP_BUF_SIZE, PAGE_EXECUTE_READWRITE, &oldProtect));
...
*/
void HookEngine::createJmp(ULONG_PTR pFun, BYTE jmpBuffer[16])
{
	//set buffer
